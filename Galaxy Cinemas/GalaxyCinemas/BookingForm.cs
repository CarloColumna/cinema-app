...
namespace GalaxyCinemas
{
    public partial class BookingForm : Form
    {
        ...
        private void UpdatePrice()
        {
            ...
            int intQuantity;
            bool validQuantity = int.TryParse(txtQuantity.Text, out intQuantity);
            decimal originalPrice = intQuantity * BASETICKETPRICE;
            // Prepare to compare original price to special prices.
            decimal finalPrice = originalPrice;
            booking.OriginalPrice = originalPrice;
            string specialName = "";

            foreach(ISpecialPlugin isp in specialPlugins)
            {
                decimal currentSpecialPrice = originalPrice;
                string currentSpecialName = "";

                if(isp.CalculateSpecial(booking, ref currentSpecialName, ref currentSpecialPrice))
                {
            ...
            // Updating the values in the booking form
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            if (IsFormValid())
            {
                ...
                MessageBox.Show("Added booking {0}", booking.BookingNumber.ToString());
            }
            else
                return;
        }
