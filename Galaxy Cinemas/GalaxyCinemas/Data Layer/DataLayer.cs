...
values (@SessionID, @Quantity, @Special, @OriginalPrice, @Discount, @FinalPrice);";
        ...
        public static List<Session> GetAllSessionsForMovie(int movieID, DateTime date)
        {
            ...
            {
                conn.Open();
                string sql = @"select SessionID, MovieID, SessionDate, CinemaNumber from [Session]
...
where SessionDate = @currentDate";
        ...
        public static Session GetSessionByID(int sessionID)
        {
            ...
            {
                conn.Open();
                string sql = @"select MovieID, SessionDate, CinemaNumber from [Session] where SessionID = @sessionID";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
        ...
        public static void AddMovie(Movie movie)
        {
            ...
            {
                conn.Open();
                string sql = @"insert into [Movie] (MovieID, Title) values (@movieID, @title)";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
